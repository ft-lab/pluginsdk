...
#pragma clang diagnostic ignored "-Wold-style-cast"
	...
	virtual float get_zoom (void* = nullptr) const = 0; // 0
	virtual sx::vec<int,2> get_whole_image_size (void* = nullptr) const = 0; // 1
	virtual sxsdk::image_interface* get_image_interface (void* = nullptr) const = 0; // 2
	virtual sxsdk::mat4 get_world_to_device_matrix (void* = nullptr) const = 0; // 3
	virtual sxsdk::mat4 get_world_to_view_matrix (void* = nullptr) const = 0; // 4
	virtual int get_number_of_lights (void* = nullptr) const = 0; // 5
	...
	virtual plugin_exporter_interface& get_plugin_exporter_interface (void* = nullptr) const = 0; // 8
	...
	virtual sxsdk::mat4 get_world_to_perspective_matrix (void* = nullptr) const = 0; // 11
	virtual int get_current_frame_deprecated (void* = nullptr) const = 0; // 12
	...
	virtual rendering_context_interface &set_progress (float progress_param, void* = nullptr) = 0; // 16
	virtual void end_progress (void* aux = 0) = 0; // 17
	virtual sxsdk::rgb_class correct (const sxsdk::rgb_class& color, void* aux = 0) = 0; // 18
	virtual bool is_animation (void* = nullptr) const = 0; // 19
	virtual void update_joint_values (void* aux = 0) = 0; // 20
	virtual shape_interface* get_current_shape_interface (void* aux = 0) = 0; // 21
	virtual sxsdk::shape_class& get_current_shape (void* = nullptr) const = 0; // 22
	virtual sxsdk::vec3 get_eye (void* = nullptr) const = 0; // 23
	virtual sxsdk::vec3 get_target (void* = nullptr) const = 0; // 24
	virtual bool get_sight (sxsdk::vec3& eye, sxsdk::vec3& direction, float x, float y, void* aux = 0) = 0; // 25
	virtual int get_total_blocks (void* = nullptr) const = 0; // 26
	...
	virtual sxsdk::mat4 get_camera_to_world_matrix (void* = nullptr) const = 0; // 28
	virtual bool is_partial_rendering (void* = nullptr) const = 0; // 29
	virtual rendering_light_class& get_light (int i, void* aux = 0) = 0; // 30
	virtual bool get_flip_face (void* = nullptr) const = 0; // 31
	virtual bool get_flip_face_shade (void* = nullptr) const = 0; // 32
	virtual sxsdk::rgb_class calculate_background_color ( const sxsdk::vec3& v, void* aux = 0) = 0; // 33
	virtual bool get_render (void* = nullptr) const = 0; // 34
	virtual int get_interlace (void* = nullptr) const = 0; // 35
	virtual proxy_shape_class& get_current_proxy_shape (void* = nullptr) const = 0; // 36
	virtual proxy_shape_class& get_root_proxy_shape (void* = nullptr) const = 0; // 37
	virtual bool get_rendering_flag ( const sxsdk::shape_class& shape, void* aux = 0) = 0; // 38
	virtual sxsdk::background_interface* get_background_interface (void* = nullptr) const = 0; // 39
	virtual sxsdk::camera_class& get_camera (void* = nullptr) const = 0; // 40
	...
	virtual bool get_active (void* = nullptr) const = 0; // 46
	virtual float get_current_frame (void* = nullptr) const = 0; // 47

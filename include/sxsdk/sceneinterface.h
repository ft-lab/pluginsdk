...
#pragma clang diagnostic ignored "-Wold-style-cast"
	virtual sxsdk::rendering_interface* get_rendering_interface (void* = nullptr) const = 0; // 0
	virtual camera_interface* get_camera_interface (void* = nullptr) const = 0; // 1
	virtual sxsdk::distant_light_interface* get_distant_light_interface (void* = nullptr) const = 0; // 2
	virtual sxsdk::background_interface* get_background_interface (void* = nullptr) const = 0; // 3
	virtual sxsdk::correction_interface* get_correction_interface (void* = nullptr) const = 0; // 4
	...
	virtual scene_interface &set_unit (sxsdk::enums::unit unit_param, void* = nullptr) = 0; // 108
	virtual sxsdk::enums::unit get_unit (void* = nullptr) const = 0; // 109
	virtual scene_interface &set_cursor_position (const sxsdk::vec3 &cursor_position_param, void* = nullptr) = 0; // 110
	virtual sxsdk::vec3 get_cursor_position (void* = nullptr) const = 0; // 111
	virtual scene_interface &set_figure_view_mode (int figure_view_mode_param, void* = nullptr); scene_interface &set_figure_view (int figure_view_mode_param) { return set_figure_view_mode(figure_view_mode_param); }  // 112
	virtual int get_figure_view_mode (void* = nullptr) const; int get_figure_view() const { return get_figure_view_mode(); }  // 113
	virtual scene_interface &set_snap (bool snap_param, void* = nullptr) = 0; // 114
	virtual bool get_snap (void* = nullptr) const = 0; // 115
	virtual scene_interface &set_grid (bool grid_param, void* = nullptr) = 0; // 116
	virtual bool get_grid (void* = nullptr) const = 0; // 117
	virtual scene_interface &set_large_cursor (bool large_cursor_param, void* = nullptr) = 0; // 118
	virtual bool get_large_cursor (void* = nullptr) const = 0; // 119
	virtual scene_interface &set_template (bool template_param, void* = nullptr) = 0; // 120
	virtual bool get_template (void* = nullptr) const = 0; // 121
	virtual scene_interface &set_color_mode (bool color_mode_param, void* = nullptr) = 0; // 122
	virtual bool get_color_mode (void* = nullptr) const = 0; // 123
	virtual sxsdk::mat4 get_user_to_world_matrix (void* = nullptr) const = 0; // 124
	virtual int get_number_of_active_shapes (void* = nullptr) const = 0; // 125
	...
	virtual float get_pixel_size (void* = nullptr) const = 0; // 129
	virtual float get_default_radius (void* = nullptr) const = 0; // 130
	...
	virtual quickrendering_interface* get_quickrendering_interface (void* = nullptr) const = 0; // 133
	virtual sxsdk::fog_interface* get_fog_interface (void* = nullptr) const = 0; // 134
	...
	virtual scene_interface &set_backdrop (bool backdrop_param, void* = nullptr) = 0; // 142
	virtual bool get_backdrop (void* = nullptr) const = 0; // 143
	...
	virtual float get_native_to_user_unit (void* = nullptr) const = 0; // 149
	virtual float get_user_to_native_unit (void* = nullptr) const = 0; // 150
	virtual sxsdk::mat4 get_world_to_user_matrix (void* = nullptr) const = 0; // 151
	virtual void load_EPSF (const char*const& from, void* aux = 0) = 0; // 152
	virtual scene_interface &set_sequence_value (float sequence_value_param, void* = nullptr) = 0; // 153
	virtual float get_sequence_value (void* = nullptr) const = 0; // 154
	...
	virtual int get_number_of_memorized_shapes (void* = nullptr) const = 0; // 159
	...
	virtual int get_total_frames (void* = nullptr) const = 0; // 176
	virtual void set_total_frames (int n, bool adjust, void* aux = 0) = 0; // 177
	virtual int get_start_frame (void* = nullptr) const = 0; // 178
	virtual scene_interface &set_start_frame (int start_frame_param, void* = nullptr) = 0; // 179
	virtual int get_end_frame (void* = nullptr) const = 0; // 180
	virtual scene_interface &set_end_frame (int end_frame_param, void* = nullptr) = 0; // 181
	virtual int get_frame_rate (void* = nullptr) const = 0; // 182
	virtual scene_interface &set_frame_rate (int frame_rate_param, void* = nullptr) = 0; // 183
	...
	virtual int get_number_of_appended_points (void* = nullptr) const = 0; // 188
	virtual void cancel_line_object (void* aux = 0) = 0; // 189
		void cancel_polygon (void* aux = 0) { cancel_line_object(); }
	virtual bool can_save (void* = nullptr) const = 0; // 190
	virtual bool can_export_foreign (void* = nullptr) const = 0; // 191
	virtual bool can_export_native (void* = nullptr) const = 0; // 192
	virtual bool can_create (void* = nullptr) const = 0; // 193
	virtual bool can_save_rendered_image (void* = nullptr) const = 0; // 194
	virtual void append_polygon_mesh_mesh (mesh_interface* mesh, void* aux = 0) = 0; // 195
	virtual bool is_modify_mode (void* = nullptr) const = 0; // 196
	...
	virtual sxsdk::shape_class* get_insertion_shape (void* = nullptr) const = 0; // 207
	...
	virtual int get_step_frames_deprecated (void* = nullptr) const = 0; // 211
	virtual scene_interface &set_step_frames_deprecated (int step_frames_deprecated_param, void* = nullptr) = 0; // 212
...
sxsdk::shape_class* get_shape_by_name_using_default (const char* name) { const bool b = true; return get_shape_by_name(name, b); }
	...
	virtual int get_active_shapes (sxsdk::shape_class* list[], void* = nullptr) const = 0; // 222
	virtual scene_interface &set_active_shapes (int n, sxsdk::shape_class* list[], void* = nullptr) = 0; // 223
	virtual bool is_rendering_thread (void* = nullptr) const = 0; // 224
	virtual rendering_context_interface* get_rendering_context_interface (void* = nullptr) const = 0; // 225
	...
	virtual sxsdk::radiosity_interface* get_radiosity_interface (void* = nullptr) const = 0; // 231
	...
	virtual sxsdk::shape_class& get_shape (void* = nullptr) const = 0; // 238
	...
	virtual float get_default_bevel_radius (void* = nullptr) const = 0; // 243
	virtual scene_interface &set_default_bevel_radius (float default_bevel_radius_param, void* = nullptr) = 0; // 244
	virtual float get_default_disk_radius (void* = nullptr) const = 0; // 245
	virtual scene_interface &set_default_disk_radius (float default_disk_radius_param, void* = nullptr) = 0; // 246
	virtual float get_default_sphere_radius (void* = nullptr) const = 0; // 247
	virtual scene_interface &set_default_sphere_radius (float default_sphere_radius_param, void* = nullptr) = 0; // 248
	virtual float get_default_light_radius (void* = nullptr) const = 0; // 249
	virtual scene_interface &set_default_light_radius (float default_light_radius_param, void* = nullptr) = 0; // 250
	...
	virtual sxsdk::animation_settings_interface* get_animation_settings_interface (void* = nullptr) const = 0; // 255
	virtual bool get_mapping_modify_mode (void* = nullptr) const = 0; // 256
	virtual scene_interface &set_mapping_modify_mode (bool mapping_modify_mode_param, void* = nullptr) = 0; // 257
	...
	virtual scene_interface &set_dirty (bool dirty_param, void* = nullptr) = 0; // 270
	virtual bool get_dirty (void* = nullptr) const = 0; // 271
	...
	virtual float get_light_scale (void* = nullptr) const = 0; // 276
	virtual scene_interface &set_light_scale (float light_scale_param, void* = nullptr) = 0; // 277
	virtual sxsdk::mat4 get_world_to_device_matrix (void* = nullptr) const = 0; // 278
	virtual sxsdk::mat4 get_world_to_view_matrix (void* = nullptr) const = 0; // 279
	virtual sxsdk::mat4 get_world_to_perspective_matrix (void* = nullptr) const = 0; // 280
	...
	virtual bool is_create_line_mode (void* = nullptr) const = 0; // 292
	virtual sxsdk::vec3 get_center (void* = nullptr) const = 0; // 293
	virtual sxsdk::keyframes_interface* get_keyframes_interface (void* = nullptr) const = 0; // 294
	virtual sxsdk::vec3 get_current_cursor_position (void* = nullptr) const = 0; // 295
	virtual void cancel_transformation (void* aux = 0) = 0; // 296
	virtual void link (void* aux = 0) = 0; // 297
	virtual sxsdk::quaternion_class get_lathe_rotation (void* = nullptr) const = 0; // 298
	virtual scene_interface &set_lathe_rotation (const sxsdk::quaternion_class &lathe_rotation_param, void* = nullptr) = 0; // 299
	virtual sxsdk::shape_class* get_local (void* = nullptr) const = 0; // 300
	virtual void set_local (sxsdk::shape_class* local, void* aux = 0) = 0; // 301
	virtual sxsdk::mat4 get_local_matrix (void* = nullptr) const = 0; // 302
	virtual scene_interface &set_local_matrix (const sxsdk::mat4 &local_matrix_param, void* = nullptr) = 0; // 303
	virtual void close (void* aux = 0) = 0; // 304
	virtual int get_number_of_metacameras (void* = nullptr) const = 0; // 305
	virtual int get_number_of_cameras (void* = nullptr) const = 0; // 306
	virtual scene_interface &set_current_camera (int current_camera_param, void* = nullptr) = 0; // 307
	virtual int get_current_camera (void* = nullptr) const = 0; // 308
	virtual camera_interface* get_nth_camera_interface (int n, void* aux = 0) = 0; // 309
	virtual scene_interface &set_sequence_mode (bool sequence_mode_param, void* = nullptr) = 0; // 310
	virtual bool get_sequence_mode (void* = nullptr) const = 0; // 311
	...
	virtual void convert_to_polygon_mesh_deprecated (int m = 1, int n = 1, void* aux = 0) = 0; // 318
	virtual void draw_figure_window (void* aux = 0) = 0; // 319
	virtual scene_interface &set_window_frame (const sx::rectangle_class &window_frame_param, void* = nullptr) = 0; // 320
	virtual sx::rectangle_class get_window_frame (void* = nullptr) const = 0; // 321
	virtual float get_fps (void* = nullptr) const = 0; // 322
	virtual sxsdk::figure_window_interface* get_figure_window_interface (void* = nullptr) const = 0; // 323
	virtual camera_class& get_camera (void* = nullptr) const = 0; // 324
	virtual void read (const char* from, void* aux = 0) = 0; // 325
	virtual scene_interface &set_pers_grid (bool pers_grid_param, void* = nullptr) = 0; // 326
	virtual bool get_pers_grid (void* = nullptr) const = 0; // 327
	virtual bool is_undo_mode (void* = nullptr) const = 0; // 328
	...
	virtual scene_interface &set_cursor_plane (int cursor_plane_param, void* = nullptr) = 0; // 332
	virtual int get_cursor_plane (void* = nullptr) const = 0; // 333
...
std::vector<sxsdk::shape_class*> get_shapes_by_name_scriptimplementation (const char* name, bool default_name = false, void* aux = 0) ;
	...
	virtual sxsdk::figure_view_interface* get_figure_view_interface (void* = nullptr) const = 0; // 339
	virtual sxsdk::display_interface* get_display_interface (void* = nullptr) const = 0; // 340
	virtual bool is_redo_mode (void* = nullptr) const = 0; // 341
	...
	virtual int get_open_time (void* = nullptr) const = 0; // 345
	virtual int get_save_time (void* = nullptr) const = 0; // 346
	virtual scene_interface &set_show_normals (bool show_normals_param, void* = nullptr) = 0; // 347
	virtual bool get_show_normals (void* = nullptr) const = 0; // 348
	virtual scene_interface &set_large_point (bool large_point_param, void* = nullptr) = 0; // 349
	virtual bool get_large_point (void* = nullptr) const = 0; // 350
	virtual void unpurge_images (void* aux = 0) = 0; // 351
	virtual scene_interface &set_selection_mode (int selection_mode_param, void* = nullptr) = 0; // 352
	virtual int get_selection_mode (void* = nullptr) const = 0; // 353
	...
	virtual const char* get_file_path (void* = nullptr) const = 0; // 357
	...
	virtual bool is_locked (void* = nullptr) const = 0; // 361
	...
	virtual bool is_motion_playing (void* = nullptr) const = 0; // 370
	...
	virtual scene_interface &set_work_plane (bool work_plane_param, void* = nullptr) = 0; // 416
	virtual bool get_work_plane (void* = nullptr) const = 0; // 417
	virtual void* get_linedrawing_interface (void* = nullptr) const = 0; // 418
	...
	virtual scene_interface &set_selection_method (int selection_method_param, void* = nullptr) = 0; // 423
	virtual int get_selection_method (void* = nullptr) const = 0; // 424
	...
	virtual sxsdk::speed_interface* get_speed_interface (void* = nullptr) const = 0; // 432
	virtual bool get_skipped (void* = nullptr) const = 0; // 433
	virtual scene_interface &set_pivot_position (sxsdk::vec3 pivot_position_param, void* = nullptr) = 0; // 434
	virtual sxsdk::vec3 get_pivot_position (void* = nullptr) const = 0; // 435
	virtual scene_interface &set_manip_type (int manip_type_param, void* = nullptr) = 0; // 436
	virtual int get_manip_type (void* = nullptr) const = 0; // 437
	...
	virtual float get_step_frames (void* = nullptr) const = 0; // 453
	virtual scene_interface &set_step_frames (float step_frames_param, void* = nullptr) = 0; // 454
	...
	virtual sxsdk::shape_class& create_primitive_text_deprecated (const char* as, int type, bool set_uv, const sxsdk::vec3& at, int axis, int flip, const sxsdk::vec3& edir, const char* text, const char* fontname, float fontsize, float space, int weight, bool italic, void* aux = 0) = 0; // 463
	virtual void save_FBX_deprecated (const char*const& in, const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 464
	virtual void save_DAE (const char*const& in, const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 465
	virtual void mark_select_face_uvs (void* aux = 0) = 0; // 466
	virtual void unmark_select_face_uvs (void* aux = 0) = 0; // 467
	virtual void create_offset_lines (float distance, const sxsdk::vec3& axis, int direction = -1, int edge_type = 1, float roundness = 1.0, int repetition = 1, float distance_slope = 0.0, int distance_to = 0, float height = 0.0, float height_slope = 0.0, int height_to = 0, bool allow_splits = false, bool delete_source = false, void* aux = 0) = 0; // 468
	virtual void do_manifold_boolean (sxsdk::smlib::boolean_operation_type bo_type, int du, int dv, int level, bool triangulate, bool divide, void* aux = 0) = 0; // 469
	virtual void save_STL (const char*const& in, const char*const& profile = "", const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 470
	virtual sxsdk::shape_class& create_primitive_text (const char* as, int type, bool set_uv, const sxsdk::vec3& at, int axis, int flip, const sxsdk::vec3& edir, const char* text, const char* fontname, float fontsize, const sxsdk::vec2& space, int weight, bool italic, bool vertical, void* aux = 0) = 0; // 471
	virtual void save_FBX (const char*const& in, const char*const& profile = "", const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 472
	virtual void save_OBJ_wp (const char*const& in, const char*const& profile = "", const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 473
	virtual void save_3DS_wp (const char*const& in, const char*const& profile = "", const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 474
	virtual void export_with_uuid_wp (const sx::uuid_class& plugin_id, const char*const& in, const char*const& profile = "", const sxsdk::exporter_settings_interface* settings = 0, void* aux = 0) = 0; // 475
	virtual void load_STL (const char*const& from, const char*const& profile = "", void* aux = 0) = 0; // 476
	virtual void fit_to_active_view (float fat_scale, void* aux = 0) = 0; // 477
	virtual void fit_to_all_views (float fat_scale, void* aux = 0) = 0; // 478
	virtual void do_shell_separation (void* aux = 0) = 0; // 479
	virtual int get_visible_shapes (sxsdk::shape_class* shapes[], bool active_only, bool shown_only, bool rendering_only, void* aux = 0) const = 0; // 480
	virtual scene_interface &set_current_workspace (const char* current_workspace_param, void* = nullptr) = 0; // 481
	virtual const char* get_current_workspace (void* = nullptr) const = 0; // 482
	virtual void begin_boolean_shapes (void* aux = 0) = 0; // 483
	virtual void add_boolean_shape (sxsdk::shape_class* shape, void* aux = 0) = 0; // 484
	virtual void end_boolean_shapes (void* aux = 0) = 0; // 485
	virtual void do_disconnected_separation (void* aux = 0) = 0; // 486
	virtual const char* get_creation_shade_version (void* = nullptr) const = 0; // 487
	virtual const char* get_last_saved_shade_version (void* = nullptr) const = 0; // 488
	virtual int get_creation_shade_build (void* = nullptr) const = 0; // 489
	virtual int get_last_saved_shade_build (void* = nullptr) const = 0; // 490
	virtual int create_wrapping_mesh (int level = 0, bool hide_source_shapes = false, float adaptive = 0.001, int non_watertight = 0, int shape_subdivision = 2, bool pre_flip = true, bool edge_optimize = false, bool separate_shells = true, bool remove_small_shells = true, void* aux = 0) = 0; // 491

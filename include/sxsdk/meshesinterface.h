...
namespace sxsdk {
	...
	public:
		...
		public:
			...
			sx::vec2 get_uv (int i, int j, void* aux =0) const { return meshes.get_uv(this->i+i, this->j+j); }
...
#pragma clang diagnostic ignored "-Wold-style-cast"
	virtual int get_m_number (void* = nullptr) const = 0; // -1
	virtual int get_n_number (void* = nullptr) const = 0; // -1
	virtual bool get_m_closed (void* = nullptr) const = 0; // -1
	virtual bool get_n_closed (void* = nullptr) const = 0; // -1
	virtual bool get_must_triangulate (void* = nullptr) const = 0; // -1
	virtual bool has_normal_deprecated (void* = nullptr) const = 0; // -1
	virtual bool is_normal_reversed (void* = nullptr) const = 0; // -1
	...
	virtual bool has_uv_coordinates (void* = nullptr) const = 0; // -1
	virtual sxsdk::vec2 get_uv_coordinates (int i, int j, void* aux = 0) = 0; // -1
	virtual bool has_uv_vectors (void* = nullptr) const = 0; // -1
	virtual uv_vector_class get_uv_vectors (int i, int j, void* aux = 0) = 0; // -1
	virtual sxsdk::image_interface* get_texture_image_interface (void* = nullptr) const = 0; // 0
